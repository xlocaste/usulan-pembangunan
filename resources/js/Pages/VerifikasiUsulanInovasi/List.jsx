...
export default function List({ daftarVerifikasi }) {
    ...
    const handleDelete = (id) => {
        if (confirm('Apakah Anda yakin ingin menghapus data verifikasi ini?')) {
            router.delete(route('verifikasi-usulan-inovasi.destroy', id));
        }
    };

    return (
        <AuthenticatedLayout user={auth.user} header="Verifikasi Usulan Inovasi">
                                ...
                                <tr key={item.id} className="hover:bg-gray-50">
                                    ...
                                    <td className="px-4 py-2 border text-center">
                                        <div className="flex justify-center items-center gap-2">
                                            <Link
                                                href={route('verifikasi-usulan-inovasi.edit', item.id)}
